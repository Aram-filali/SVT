          ...
          {groups.map((item) => (
            <div key={`${item.id}-${item.child.id}`} className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              <div className="flex gap-2 border-t pt-3">
                <Link
                  href={`/my-children/${item.child.id}/evaluations`}
                  className="flex-1 text-center text-xs font-semibold px-3 py-2 bg-violet-50 text-violet-700 rounded hover:bg-violet-100 border border-violet-200 transition"
                >
                  📝 Évaluations & Notes
                </Link>
                <Link
                  href={`/my-children/${item.child.id}/progress`}
                  className="flex-1 text-center text-xs font-semibold px-3 py-2 bg-fuchsia-50 text-fuchsia-700 rounded hover:bg-fuchsia-100 border border-fuchsia-200 transition"
                >
                  📈 Progression
                </Link>
              </div>
